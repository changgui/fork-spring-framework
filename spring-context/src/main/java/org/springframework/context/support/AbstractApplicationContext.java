...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				// 钩子方法，此处没有实现，springmvc中有
				// 作用是在BeanFactory准备工作完成后做一些定制化的处理，
				// 一般结合BeanPostProcessor接口的实现类一起使用，注入一些重要资源（类似Application的属性和ServletContext的属性）。
				...
				// 此处无任何实现，但是在springboot中启动了web容器
				onRefresh();
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		// 初始化BeanFactory并进行XML读取，并将得到的BeanFactory记录在当前实体的属性中
		refreshBeanFactory();
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 六个Aware处理类 EnvironmentAware|EmbeddedValueResolverAware|ResourceLoaderAware|ApplicationEventPublisherAware|MessageSourceAware|ApplicationContextAware
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		if (beanFactory.containsBean(CONVERSION_SERVICE_BEAN_NAME) && beanFactory.isTypeMatch(CONVERSION_SERVICE_BEAN_NAME, ConversionService.class)) {
			// 如果自定义配置了转换器此处就能拿到并且放入beanFactory中
		...
		// 在PropertySourcesPlaceholderConfigurer中会做赋值操作
		...
		// 冻结所有的bean定义，为防止意外发生，在实例化bean之前禁止对beanDefinition进行修改
	...
	protected void finishRefresh() {
		...
		// 初始化LifecycleProcessor，默认DefaultLifecycleProcessor
		initLifecycleProcessor();

		// Propagate refresh to lifecycle processor first.
		// 刷新处理器，上下文的通知，例如自启动的组件
