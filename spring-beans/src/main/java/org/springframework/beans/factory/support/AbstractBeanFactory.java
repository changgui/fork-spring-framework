...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType, @Nullable final Object[] args, boolean typeCheckOnly) throws BeansException {
		...
		if (sharedInstance != null && args == null) {
			...
			// 如果bean实现了FactoryBean接口，那么在spring的生命周期中会创建两个对象
			// 一是实现了FactoryBean接口的子类，spring自己管理
			// 二是getObject返回的对象，在此处创建，创建完毕后在DefaultListableBeanFactory的factoryBeanObjectCache中存储
			bean = getObjectForBeanInstance(sharedInstance, name, beanName, null);
		} else {
			// Fail if we're already creating this bean instance:
			// We're assumably within a circular reference.
			// prototype模式的循环依赖检测，spring解决不了，直接抛异常
			...
			// 此处有个父子容器的概念
			// 有父容器且自己的beanDefinitionMap没有这个beanDefinition，就会去父容器里查找并实例化
	...
	protected RootBeanDefinition getMergedBeanDefinition(String beanName, BeanDefinition bd, @Nullable BeanDefinition containingBd) throws BeanDefinitionStoreException {
		...
		synchronized (this.mergedBeanDefinitions) {
			...
			if (mbd == null) {
				// 如果bd的parentName为空表示没有父定义，不需要进行合并操作
			...
			// 最终都是返回一个RootBeanDefinition
			// 是一个从GenericBeanDefinition到RootBeanDefinition的过程
			return mbd;
		}
	}
